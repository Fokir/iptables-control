...
export class UnixStrategy implements IptablesStrategyInterface {
  ...
  private async execPreRouting(
    ...
    await execPromise(
      `iptables -A PREROUTING -t nat -d ${targetIp} -p ${protocol} --dport ${port} -j DNAT --to-dest ${destinationIp}:${port}`,
    );
  }
  ...
  private async execPostRouting(
    ...
    await execPromise(
      `iptables -A POSTROUTING -t nat -d ${targetIp} -p ${protocol} --dport ${port} -j SNAT --to-source ${destinationIp}`,
    );
  }
}
